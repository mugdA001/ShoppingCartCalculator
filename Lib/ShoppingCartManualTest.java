...
public class ShoppingCartManualTest {
    ...
    public static void run() {
        ...
        // Test 4: คำนวณราคาแบบมีส่วนลด BOGO
         ArrayList<CartItem> bogoCart = new ArrayList<>();
        bogoCart.add(new CartItem("BOGO", "Bread", 25.0, 4)); // 50
        bogoCart.add(new CartItem("BOGO", "Milk", 15.0, 3));   // 30
        ...
        if (total4 == 80.0) {
            ...
            failedCount++;
             ...
             // Test 5: คำนวณราคาแบบมีส่วนลด BULK
         ArrayList<CartItem> bulkCart = new ArrayList<>();
        bulkCart.add(new CartItem("BULK", "Bread", 25.0, 10)); // 250-10% = 225
        bulkCart.add(new CartItem("BULK", "Milk", 15.0, 5));   // 75
        double total5 = ShoppingCartCalculator.calculateTotalPrice(bulkCart);
        if (total5 == 300.0) {
            System.out.println("PASSED: BULK cart total is correct (300.0)");
            passedCount++;
        } else {
            System.out.println("FAILED: BULK cart total expected 300.0 but got " + total5);
            failedCount++;
        } 
